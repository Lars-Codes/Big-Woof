...
export default memo(function ClientItem({ client }) {
  ...
  const handleClientPress = useCallback(() => {
    if (deleteMode) {
      dispatch(toggleClientInDeleteSet(client.client_id));
  ...
  return (
    ...
    >
      ...
      <View className={`${isClientInDeleteSet ? 'bg-gray-100' : 'bg-white'}`}>
        <TouchableOpacity
